    ...
    <nav className={`fixed top-0 left-0 w-full z-100 transition-all duration-500 ${isScrolling ?'py-4':'py-10' }`}>
        <div className='container max mx-auto px-6'>
            <div className={`flex items-center justify-between px-8 py-6 rounded-full border transition-all duration-500 ${isScrolling? "bg-mtech-dark/80 backdrop-blur-2xl border-white/10 shadow-2xl shadow-black/50":"bg-transparent border-transparent"}`}>
                ...
                <img src={logo} alt='mtech lab logo'  className='h-20 w-auto invert brightness-0 transition-transform duration-500 relative z-10 group-hover:scale-105 '/>
                </Link>
                ...
                <div className='flex items-center gap-6'>
                  ...
                  <div className='w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-mtech-orange group-hover:border-mtech-orange transition during-500 ' >
                    <ArrowUpRight size={16} className='text-white'/>
                  </div>
                  </Link>
                  <button onClick={()=>setIsMenuOpen(!isMenuOpen)}
                    className='w-12 h-12 rounded-full bg-white/5 border-white/10 flex items-center justify-center text-white hover:bg-mtech-orange transition-colors'>
